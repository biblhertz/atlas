...
function buildGeometry() {
  ...
  removeLoaderScene();
  console.log('removed loader, loading larger atlas files')
  loadLargeAtlasFiles();
  console.log('loaded larger atlas files')
}
...
function getAtlasImages(atlasIdx, materialIdx) {
  ...
  imageDataKeys.slice(startImage, endImage).forEach(function(d) {
    ...
    img.width *= 4;   //THIS also needs to be changed when you change resolution of images to e.g. 64 or 128 etc
    img.height *= 4;
    img.xOffset *= 4;
    img.yOffset *= 4;
    // Get atlas information for this image
...
function slideBlock(elem) {
  ...
  setTimeout(removeElem.bind(null, elem), 1000)     //this seems to raise an error
}
...
function flyTo(x, y, z) {
  ...
  var target = { x: x, y: y, z: z + 500 }   //EDIT ZOOM HERE
